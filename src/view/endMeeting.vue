<template>
  <div class="box" ref="getBox">
    <van-tabs v-model="tabChangeNum" @click="tabChange" color="darkorange">
      <van-tab title="我参与"></van-tab>
      <van-tab title="我相关"></van-tab>
    ...
    <van-pull-refresh v-else v-model="isLoading" @refresh="onRefresh" success-text="刷新成功">
      <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
        <meetingItem v-for="(item,index) in list" :key="index" :item="item" @getItemId="getItemMsg"></meetingItem>
      </van-list>
    </van-pull-refresh>
...
export default {
  ...
  watch: {
    $route: {
      handler(to, from) {
        if (from && to.path == "/entrance/endMeeting") {
          this.list = [];
          this.pageSize = {
            current: 0,
            size: 10
  ...
  methods: {
   ...
   //返回顶部
    ...
    // 获取与我相关的数据
    getUiPartakeMeeting(page) {
      getPartakeMeeting(page).then(res => {
        console.log(res);
        if (res.status === 200) {
          // this.list = res.data.data.records
          let newRes = JSON.parse(JSON.stringify(res.data.data));
          this.list = this.list.concat(newRes.records);
          // 加载状态结束
          this.loading = false;

          // // 数据全部加载完成
          if (this.list.length == newRes.total) {
            this.finished = true;
          }
        }
      });
    ...
    //获取我参与的
    getUiAboutMeMeeting(page) {
      let data = {
        current: page.current,
        size: page.size,
        id: this.$store.state.base.userInfo.user_id
      };
      getAboutMeMeeting(data).then(res => {
        if (res.status === 200) {
          let newAboutRes = JSON.parse(JSON.stringify(res.data.data));
          this.list = this.list.concat(newAboutRes.records);
          // 加载状态结束
          this.loading = false;

          // // 数据全部加载完成
          if (this.list.length == newAboutRes.total) {
            this.finished = true;
          }
        }
      });
    ...
    //其他
    getUiOtherMeeting(page) {
      getOtherMeeting(page).then(res => {
        console.log(res);
        if (res.status === 200) {
    ...
    getItemMsg(id) {
      ...
      this.$router.push({
        ...
        query: {
          id: id,
          meetingType:2 //0=未开始、1=进行中、2=已结束
        }
      });
    },
    onLoad() {
      ...
      if (this.tabChangeNum == 1) {
        this.getUiAboutMeMeeting(this.pageSize);
      } else if (this.tabChangeNum == 2) {
        this.getUiOtherMeeting(this.pageSize);
      } else {
        this.getUiPartakeMeeting(this.pageSize);
      }
      // }, 1000);
    },
