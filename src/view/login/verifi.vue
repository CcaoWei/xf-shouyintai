<template>
  <div class="login" v-if="$route.path == '/entrance/verifi'">
    <div class="login-top">
    ...
    <van-cell-group class="login-form">
      <van-field class="login-input phone" v-model="userInfo.user_name" center readonly type="number"></van-field>
    ...
    <van-button class="sub-btn" type="danger" @click="goReset">下一步</van-button>
  </div>
      ...
      goReset() {
        if (!this.pwdValue) {
          ...
          idenCode(data).then(res => {
            if (res.status == 200) {
              this.$router.push({
                path: "/entrance/verifi/updatePassWord",
                query: {
                  phone: this.userInfo.user_name
                }
              });
