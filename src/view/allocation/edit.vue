      ...
      <div class="s-cellList">
        ...
        <div class="cell-item" @click="showPopup1">
          ...
          <div class="cell-right">
            <span>{{ shopName === '' ? "请选择调拨店铺" : shopName }}</span>
...
import {orderGoodsSearch, addOutOrPut, getShopBySpu,getGoodsBySpu} from "@/api/stock";
  ...
  methods: {
    ...
    getData() {
      ...
      orderGoodsSearch(params).then(res => {
        ...
        let spus = []
        this.goods.forEach(item=>{
          spus.push(item.spuId)
        })

        getShopBySpu({spus:JSON.stringify(spus)}).then(res => {
    ...
    onSelect(item) {
      ...
      this.data.shopId = item.id;

      getGoodsBySpu({deptId:item.id,spus:JSON.stringify(spus)}).then(res => {
    ...
    initial() {
      this.$router.push('/entrance/allocation')
    }
  },
  watch: {}
};
