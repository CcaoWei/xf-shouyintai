  ...
  <div class="baseBody">
    ...
    <div class="form">
      <div class="form-body">
        ...
        <van-field class="form-textarea" rows="2" autosize label="备注" type="textarea" maxlength="50" placeholder="请输入备注" show-word-limit v-model="remark" input-align="right" />
      </div>
    </div>
    <div class="cardArea">
      <div class="card-body">
        <van-card class="card-item">
          <div slot="desc">
            ...
            <van-swipe-cell v-for="(item,index) in goods" :key='index' :before-close="beforeClose" class="good-item shadow-row">
              <template slot="right">
  ...
  methods: {
    search() {
      getGoodsByCode(this.goodCode).then(res => {
        if (res.data.data && res.data.data.id && res.data.data.id != -1) {
...
.baseBody {
  ...
  .shadow-row-more {
    box-shadow: 0 1px 0 0 rgba(143, 155, 179, 0.2);
    border-bottom: 1px solid rgba(143, 155, 179, 0.2);
  }
