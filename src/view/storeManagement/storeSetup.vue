<template>
  <div class="add-store-box">
    ...
    <div class="add-store">
      <div class="add-btn" @click="setSure">完成</div>
    </div>
  </div>
</template>

<script>
import { setStore, uploadImg, storeDetail, shopowner } from "@/api/store";
  ...
  methods: {
    getDetail() {
      storeDetail().then(res => {
        if (res.status == 200) {
          this.storeInfo = JSON.parse(JSON.stringify(res.data.data));
          if (this.storeInfo.shopOwner) {
            this.shopownerList.forEach(item => {
              if (this.storeInfo.shopOwner === item.id) {
                this.shopOwnerValue = item.name;
              }
            });
          }
          if (res.data.data.pic) {
            let picArr = res.data.data.pic.split(",");
    ...
    setType(value) {
      ...
      this.shopOwnerValue = value;
      if(value == '不设店长'){
        this.storeInfo.shopOwner = null;
        this.storeInfo.defaultId = this.storeInfo.user.id;
      }
      this.showType = false;
    },
    setSure() {
          ...
          if (this.isCanSavePic) {
            uploadImg(param).then(res => {
              if (res.status == 200) {
                ...
                setStore(this.storeInfo).then(() => {
                  Toast("更新成功");
                  this.$router.push("/entrance/storeManagement");
                });
              }
            });
          } else {
            ...
            this.files.forEach(item => {
              if (item.url) {
                this.storeInfo.pic += item.url + ",";
              }
            });
            this.storeInfo.pic = this.storeInfo.pic.substring(0,this.storeInfo.pic.length-1);
            setStore(this.storeInfo).then(res => {
              if (res.status == 200) {
                Toast("更新成功");
                this.$router.push("/entrance/storeManagement");
              }
