<template>
  <div class="baseBody" v-if="$route.path === '/entrance/stockOrder'">
        ...
        <div class="form" v-for="(item,index) in data" :key="index" @click="goDetail(item.id)">
          <div class="form-body">
            <van-row class="card-row shadow-row">
              <van-col class="font-title" span="6">订货单号</van-col>
              <van-col class="font-h" span="18">{{ item.id }}</van-col>
            </van-row>
            <van-row class="card-row shadow-row">
              <van-col class="font-title" span="7">订货单名称</van-col>
              <van-col class="font-h" span="17">{{ item.orderName }}</van-col>
            </van-row>
            <van-row class="card-row shadow-row">
              <van-col class="font-title" span="12">订货单状态</van-col>
    ...
    <div class="btn-area">
      <button class="btn-edit" @click="addSure">新建订货单</button>
    </div>
  </div>
...
export default {
  ...
  activated() {
    ...
    if (this.$route.path === '/entrance/stockOrder') {
      this.getData()
    }
  },
  watch: {},
  methods: {
    ...
    addSure() {
      this.$router.push('stockOrder/add')
    }
  }
};
</script>
