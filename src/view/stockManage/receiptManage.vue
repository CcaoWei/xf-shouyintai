  ...
  <div class="rm-box">
      ...
      <div class="s-cellList">
        <div class="cell-item" @click="showPopup1">
          ...
          <div class="cell-right">
            <span>{{ reason === '' ? "请选择入库原因" : reason }}</span>
            <van-icon name="arrow"></van-icon>
          </div>
        </div>
        <div class="cell-item" v-if="reason === '订货入库' || reason === '调拨入库'" @click="showPopup3">
          <span style="width: 7rem; text-align: left;">入库单据</span>
          <div class="cell-right">
            <span>{{ orderName === '' ? "请选择入库单据" : orderName }}</span>
            <van-icon name="arrow"></van-icon>
          </div>
        </div>
    ...
    <div class="foot-area">
      <div class="bottom-btn">
        <span>共 <span class="quantity">{{ quantity ? quantity : 0 }}</span> 件商品入库</span>
        <div class="btn" @click="report()">入库</div>
      </div>
    </div>
    <van-dialog class="dialog" @close="initial" v-model="dialogShow" :show-confirm-button=false closeOnClickOverlay>
      <van-icon name="passed"/>
      <div class="dialogText">入库成功</div>
    </van-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    initial() {
      this.$store.commit("IN_ORDERSMSG", {});
      history.go(0);
    }
  },
  watch: {}
