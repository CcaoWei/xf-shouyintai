<template>
  <div class="base-box" v-if="$route.path == '/entrance/goodsSetting/goodsUnit'">
    <div class="card-area">
      <div v-if="data.length">
        <div class="card-box" v-for="(item,index) in data" :key="index">
          ...
          <div class="good-msg" v-if="item.isDefault">默认新增商品单位</div>
        </div>
      </div>
      <no-goods v-else></no-goods>
    </div>
    <div class="btn-area">
      <button class="btn-edit" @click="add">新增计量单位</button>
    </div>
  </div>
...
<script>
  import {unitList,deleteUnit} from "@/api/goods";
  ...
  export default {
    ...
    activated() {
      if(this.$route.path == '/entrance/goodsSetting/goodsUnit'){
        this.getData();
      }
    },
    methods: {
      ...
      add() {
        this.$router.push({
          path: "/entrance/goodsSetting/goodsUnit/add",
          query: {
            item: {
              id:null,
              name:null,
      ...
      edit(item) {
        this.$router.push({
          path: "/entrance/goodsSetting/goodsUnit/edit",
          query: {
            item: item
          }
        });
